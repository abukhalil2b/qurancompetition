                        ...
                        <tbody>
                            @forelse ($competitions as $item)
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-2 border">{{ __($item->student_status) }}</td>
                                    <td class="px-4 py-2 border">{{ $item->present_at }}</td>
                                    <td class="px-4 py-2 border">{{ $item->level }}</td>
                                    <td class="px-4 py-2 border">
                                        <a class="bg-purple-100 border border-purple-700 text-purple-700 rounded px-3" href="{{ route('student.choose_questionset', $item->id) }}">الباقات</a>
                                    </td>
                                    <td class="px-4 py-2 border">
                                       <div class="px-4 py-2 border"> المجموع {{ $item->grand_total }}</div>
                                       <div class="px-4 py-2 border"> عدد المحكمين {{ $item->judge_count }}</div>
                                       <div class="px-4 py-2 border"> المتوسط {{ $item->judge_count > 0 ?  $item->grand_total/$item->judge_count : 0}}</div>
